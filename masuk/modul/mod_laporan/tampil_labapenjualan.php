...
$no=1;
$penjualan = $db->query("select * from trkasir where tgl_trkasir between '$tgl_awal' and '$tgl_akhir' and shift in ($shift) order by id_carabayar ");
while ($jual = $penjualan->fetch_array()) {
	...
	while($det=$detail->fetch_array()){
		...
		$mdl = $db->query("select hrgsat_barang from barang where id_barang='$det[id_barang]' ");
		$mdl1 = $mdl->fetch_array();
	...
	$sub=$db->query("select sum(profit) as total,
	...
	$grand = $db->query("select ttl_trkasir from trkasir where kd_trkasir='$jual[kd_trkasir]' ");
...
$sub=$db->query("select sum(hrgttl_dtrkasir) as tx
							from trkasir_detail join trkasir on(trkasir_detail.kd_trkasir=trkasir.kd_trkasir)
							where shift in ($shift) and trkasir.tgl_trkasir between '$tgl_awal' and '$tgl_akhir' ");
$subt = $sub->fetch_array();
