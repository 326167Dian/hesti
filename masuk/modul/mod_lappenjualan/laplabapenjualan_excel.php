...
<table border="1">
    ...
    while($value=mysqli_fetch_array($query)):
        ...
        $no++;
    endwhile;
    $ttldet = $db->query("select sum(hrgttl_dtrkasir) as totaldet, sum(profit) as untung from trkasir_detail 
                                    JOIN trkasir ON (trkasir.kd_trkasir = trkasir_detail.kd_trkasir)
                                    where tgl_trkasir between '$tgl_awal' AND '$tgl_akhir'");
    $tdetail = $ttldet->fetch_array();

    $ttljual = $db->query("select sum(ttl_trkasir) as totaltx from trkasir 
                        where tgl_trkasir between '$tgl_awal' AND '$tgl_akhir' ");
    $total = $ttljual->fetch_array();
