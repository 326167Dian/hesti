...
?>
			...
			<div class="box box-primary box-solid">
				...
				<div class="box-body table-responsive">
					...
					$lupa = $_SESSION['level'];
					if ($lupa == 'pemilik') {
                                echo " <a class='btn  btn-primary btn-flat' href='modul/mod_laporan/cetak_barang_excel.php' target='_blank'>EXPORT TO EXCEL</a>                         <a class='btn  btn-info btn-flat' href='?module=barang&act=formupdate'>FORM UPDATE</a>                
									        ";
                            }
                    ?>        
...
<?php
        ...
        case "formupdate":
            ...
            <div class="box box-primary box-solid">
				...
				<div class="box-body table-responsive">
					<a class='btn  btn-success btn-flat' href='?module=barang&act=tambah'>UPDATE ALL DATA</a>
					<a class='btn  btn-danger btn-flat' href='?module=barang&act=tambah'>CANCEL</a>
					
					<hr>
					
					<table id="tesdata" class="table table-bordered table-striped">
						<thead>
							<tr>
								<th>No</th>
								<th>Kode</th>
								<th>Nama Barang</th>
								<th style="text-align: right; ">HNA</th>
								<th style="text-align: right; ">Harga Beli </th>
								<th style="text-align: right; ">Konversi </th>
								<th style="text-align: right; ">Harga Jual (R)</th>
								<th style="text-align: right; ">Harga Jual (Me)</th>
								<th style="text-align: right; ">Harga Jual (MP)</th>
                        ...
                        <tbody>
                            <?php
                                $getdata = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM barang ORDER BY id_barang ASC");
                                $no=1;
                                while($row = mysqli_fetch_array($getdata)):
                            ?>
                            <tr>
                                <td><input type="checkbox" id="check__<?=$no?>" name="check[]"><?=$no++?></td>
                                <td><?=$row['kd_barang']?></td>
                                <td><?=$row['nm_barang']?></td>
                                <td>
                                    <input id="hna_<?=$no?>" name="hna[]" value="<?=format_rupiah($row['hna'])?>" style="text-align:right; width:60px">
                                </td>
                                <td>
                                    <input id="hrgsat_barang_<?=$no?>" name="hrgsat_barang[]" value="<?=format_rupiah($row['hrgsat_barang'])?>" style="text-align:right; width:60px">
                                </td>
                                <td>
                                    <input id="konversi_<?=$no?>" name="konversi[]" value="<?=format_rupiah($row['konversi'])?>" style="text-align:right; width:60px">
                                </td>
                                <td>
                                    <input id="hrgjual_barang_<?=$no?>" name="hrgjual_barang[]" value="<?=format_rupiah($row['hrgjual_barang'])?>" style="text-align:right; width:60px">
                                </td>
                                <td>
                                    <input id="hrgjual_barang1_<?=$no?>" name="hrgjual_barang1[]" value="<?=format_rupiah($row['hrgjual_barang1'])?>" style="text-align:right; width:60px">
                                </td>
                                <td>
                                    <input id="hrgjual_barang2_<?=$no?>" name="hrgjual_barang2[]" value="<?=format_rupiah($row['hrgjual_barang2'])?>" style="text-align:right; width:60px">
                                </td>
                                <td>
                                    <input id="diskon_<?=$no?>" name="diskon[]" value="<?=format_rupiah($row['diskon'])?>" style="text-align:right; width:60px">
                                </td>
                                <td>
                                    <a href="#" class="btn btn-sm btn-primary" id="update-row">
                                        UPDATE ROW</a>
                                </td>
                            </tr>
                            <?php
...
// 					"className": 'text-right',
// 					"render": function(data, type, row) {
// 						return "<input type='text' id='hna[]' name='hna[]' value='"+formatRupiah(data)+"' style='text-align:right; width:60px'>";
// 					}
// 				},
// 				{
// 					"data": "hrgsat_barang",
// 					"className": 'text-center',
// 					"render": function(data, type, row) {
// 						return "<input type='text' id='hrgsat_barang[]' name='hrgsat_barang[]' value='"+formatRupiah(data)+"' style='text-align:right; width:60px'>";
// 					}
// 				},
// 				{
// 					"data": "konversi",
// 					"className": 'text-center',
// 					"render": function(data, type, row) {
// 						return "<input type='text' id='konversi[]' name='konversi[]' value='"+formatRupiah(data)+"' style='text-align:center; width:60px'>";
// 					}
// 				},
// 				{
// 					"data": "hrgjual_barang1",
// 					"className": 'text-center',
// 					"render": function(data, type, row) {
// 						return "<input type='text' id='hrgjual_barang1[]' name='hrgjual_barang1[]' value='"+formatRupiah(data)+"' style='text-align:right; width:60px'>";
// 					}
// 				},
// 				{
// 					"data": "hrgjual_barang2",
// 					"className": 'text-center',
// 					"render": function(data, type, row) {
// 						return "<input type='text' id='hrgjual_barang2[]' name='hrgjual_barang2[]' value='"+formatRupiah(data)+"' style='text-align:right; width:60px'>";
// 					}
// 				},
// 				{
// 					"data": "hrgjual_barang3",
// 					"className": 'text-center',
// 					"render": function(data, type, row) {
// 						return "<input type='text' id='hrgjual_barang3[]' name='hrgjual_barang3[]' value='"+formatRupiah(data)+"' style='text-align:right; width:60px'>";
// 					}
// 				},
// 				{
// 					"data": "diskon",
// 					"className": 'text-center',
// 					"render": function(data, type, row) {
// 						return "<input type='text' id='diskon[]' name='diskon[]' value='"+formatRupiah(data)+"' style='text-align:center; width:60px'>";
// 					}
// 				},
// 				{
// 					"data": "aksi",
// 					"render": function(data, type, row) {
// 						var btn = "<a class='btn btn-primary btn-xs' href='#'>UPDATE ROW</a>";
...
// 		});
	...
	$('#tesdata td').on('keyup','input[name="hna"]', function(){
	    console.log(this);
	});
</script>